...
( function () {
	 ...
	 */
	const LuShu = BMapLib.LuShu = function ( map, path, opts ) {
	...
	LuShu.prototype.start = function () {
		let meThis = this;
		let len = meThis._path.length;
		//不是第一次点击开始,并且小车还没到达终点
		if ( meThis.i && meThis.i < len - 1 ) {
			//没按pause再按start不做处理
			if ( !meThis._fromPause ) {
				return;
			} else if ( !meThis._fromStop ) {
				//按了pause按钮,并且再按start，直接移动到下一点
				//并且此过程中，没有按stop按钮
				//防止先stop，再pause，然后连续不停的start的异常
				meThis._moveNext( ++meThis.i );
			}
		} else {
			//第一次点击开始，或者点了stop之后点开始
			meThis._addMarker();
			//等待marker动画完毕再加载infowindow
			meThis._timeoutFlag = setTimeout( function () {
				meThis._addInfoWin();
				if ( meThis._opts.defaultContent === '' ) {
					meThis.hideInfoWindow();
				}
				meThis._moveNext( meThis.i );
			}, 400 );
		}
	...
	LuShu.prototype.getInfoWindow = function () {
		return getComputedStyle( this._overlay._div ).getPropertyValue( 'visibility' ) === 'visible';
	};
	LuShu.prototype.clearAll = function () {
		this.stop();
	...
	baidu.object.extend( LuShu.prototype, {
		_addMarker : function () {
			...
			//移除之前的overlay
			this._overlay && this._map.removeOverlay( this._overlay );
			const marker = new BMap.Marker( this._path[ 0 ] );
		...
		_setInfoWin : function ( pos ) {
			...
			const index = me._troughPointIndex( pos );
			if ( index !== -1 ) {
				clearInterval( me._intervalFlag );
		...
		_clearTimeout : function () {
			for (const i in this._setTimeoutQuene) {
		...
		_troughPointIndex : function ( markerPoi ) {
			let t = this._opts.landmarkPois, distance;
	...
	CustomOverlay.prototype.initialize = function ( map ) {
		const div = this._div = baidu.dom.create( 'div', {style : 'border:solid 1px #ccc;width:auto;min-width:50px;text-align:center;position:absolute;background:#fff;color:#000;font-size:12px;border-radius: 10px;padding:5px;white-space: nowrap;'} );
	...
	baidu.object.extend( CustomOverlay.prototype, {
		...
		setPosition : function ( poi, markerSize ) {
			...
			const px = this._map.pointToOverlayPixel( poi ),
